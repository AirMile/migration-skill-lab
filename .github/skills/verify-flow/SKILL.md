...
10. The orchestrator may automatically start a fresh isolated `debug-flow`
    agent only for `repairable`. A `repaired` debug result must return to a new
    independent verification attempt; never reuse debug context as proof.

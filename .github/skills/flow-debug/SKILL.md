---
name: flow-debug
description: Diagnose and repair one failed or repairable BLOCKED React-to-Angular migration flow inside the write scope its contract declares, then hand off to a fresh independent re-verification. Requires a high-capability coding model. Use only with /flow-debug.
---
